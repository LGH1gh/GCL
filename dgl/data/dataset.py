...
def split_data(data: Dataset,
    ...
    return GraphDataset(train), GraphDataset(val), GraphDataset(test)

def load_dataloader(args, data_name):
    data_info = DATA_INFO[data_name]
    data_path = f'{args.data_dir}/{data_name}.csv'
    data = pd.read_csv(data_path)
    data = data[data_info['columns']]
    data[data_info['columns'][0]] = data[data_info['columns'][0]].apply(smiles2dgl)
    data.dropna(axis=0, how='any', inplace=True, subset=[data_info['columns'][0]])
    graphs = data[data_info['columns'][0]].values.tolist()
    labels = torch.tensor(data[data_info['columns'][1:]].values.tolist())
    dataset = GraphDataset(graphs, labels)
    dataset.shuffle(args.seed)

    train_dataset, val_dataset, test_dataset = split_dataset(dataset, frac_list=args.split_size)
    ...
    train_dataloader = GraphDataLoader(train_dataset, batch_size=args.batch_size, drop_last=True, num_workers=0)
    test_dataloader = GraphDataLoader(test_dataset, batch_size=args.batch_size, num_workers=0)
    val_dataloader = GraphDataLoader(val_dataset, batch_size=args.batch_size, num_workers=0)
